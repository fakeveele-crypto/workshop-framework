...
@section('title','Input Barang - Datatables CRUD')
...
@section('content')
  <div class="page-header d-flex justify-content-between align-items-center">
    <h3 class="page-title">Input Barang (Datatables CRUD)</h3>
    <div class="d-flex gap-2">
      <a href="{{ route('inputbarang.datatables') }}" class="btn btn-outline-secondary">Buka Versi Non-CRUD</a>
      <a href="{{ route('inputbarang.html.crud') }}" class="btn btn-outline-primary">Buka Versi HTML Table</a>
    </div>
  </div>
  ...
  <div class="card mb-4">
    <div class="card-body">
      <form action="{{ route('inputbarang.store') }}" method="POST">
        @csrf
        <input type="hidden" name="redirect_to" value="datatablesCrud">
...
@push('scripts')
  ...
  <script>
    $(function () {
      const table = $('#inputBarangTable').DataTable({
      ...
      $('#inputBarangTable tbody').on('click', 'tr[data-id-barang]', function () {
