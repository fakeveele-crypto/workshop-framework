...
@section('content')
    <div class="page-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('modul5.pos.ajax') }}" class="btn btn-info">Buka Versi jQuery Ajax</a>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        (function () {
            ...
            if (!kodeInput || !namaInput || !hargaInput || !jumlahInput || !btnTambahkan || !btnBayar || !cartTableBody || !window.axios) {
                return;
            }
            ...
            const lookupBarang = function () {
                ...
                window.axios.get(endpoints.findBarang, {
                    params: { kode: kode },
                })
                    .then(function (response) {
                        ...
                        currentItem = {
                            kode: String(data.data.kode),
                            nama: String(data.data.nama),
                    ...
                    .catch(function (error) {
                        const message = error?.response?.data?.message || 'Kode barang tidak ditemukan.';
                        if (window.Swal) {
                            window.Swal.fire({
                                icon: 'warning',
            ...
            const checkout = function () {
                ...
                window.axios.post(endpoints.checkout, payload, {
                    headers: {
                    ...
                    .then(function (response) {
                        if (window.Swal) {
                            window.Swal.fire({
                                icon: 'success',
                                title: 'Berhasil',
                                text: response?.data?.message || 'Pembayaran transaksi berhasil disimpan.',
                            });
                        }
                    ...
                    .catch(function (error) {
                        const message = error?.response?.data?.message || 'Terjadi kesalahan saat menyimpan transaksi.';
                        if (window.Swal) {
                            window.Swal.fire({
                                icon: 'error',
